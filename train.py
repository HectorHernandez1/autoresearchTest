...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.c_fc = nn.Linear(config.n_embd, FFN_MULT * config.n_embd, bias=False)
        self.c_proj = nn.Linear(FFN_MULT * config.n_embd, config.n_embd, bias=False)
...
# Optimization
TOTAL_BATCH_SIZE = 3 * 131072 # ~393K tokens per optimizer step, 3 accum steps
FFN_MULT = 3            # MLP expansion ratio (default 4 in original code)
