...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        B, T, C = x.size()
        # Fused QKV via cat-in-forward (weights stay separate so Muon's
        # per-shape LR scaling is unchanged; compile fuses the cat+matmul).
        ...
        q = qkv[..., :q_end].view(B, T, self.n_head, self.head_dim)
        k = qkv[..., q_end:k_end].view(B, T, self.n_kv_head, self.head_dim)
        v = qkv[..., k_end:].view(B, T, self.n_kv_head, self.head_dim)
...
class GPT(nn.Module):
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=embedding_params, lr=embedding_lr * dmodel_lr_scale, betas=(0.8, 0.9), eps=1e-10, weight_decay=0.0),
            dict(kind='adamw', params=value_embeds_params, lr=embedding_lr * dmodel_lr_scale * 2.0, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.0),
            dict(kind='adamw', params=resid_params, lr=scalar_lr * 0.01, betas=(0.96, 0.95), eps=1e-10, weight_decay=0.0),
            dict(kind='adamw', params=x0_params, lr=scalar_lr * 0.5, betas=(0.96, 0.95), eps=1e-10, weight_decay=0.0),
        ]
...
# Optimization
TOTAL_BATCH_SIZE = 49152 # 48K tokens per optimizer step (1.5x 2^15)
...
WARMDOWN_RATIO = 0.93   # fraction of time budget for LR warmdown
FINAL_LR_FRAC = 0.05    # final LR as fraction of initial

# Model size
DEPTH = 7               # number of transformer layers
DEVICE_BATCH_SIZE = 24   # per-device batch size (reduced for 32GB VRAM on RDNA4)
...
print(f"depth:            {DEPTH}")
# DO NOT REMOVE — used by generate.py for before/after comparison
